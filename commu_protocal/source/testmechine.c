...
extern char* encrypt_and_sign_string(const char* plaintext);
extern char* decrypt_and_verify_string(const char* encryptedAndSigned);
...
// Function to destroy all child processes
    ...
    switch (cmd[0]) {
            ...
            if (child.pid == 0) {  // Child process
                ...
                while (1) {
                    ...
                    if (n <= 0) {
                        ...
                        buffer[n] = '\0';
                        char* decrypted = decrypt_and_verify_string(buffer);
                        if (decrypted) {
                            write_to_pipe(output_fd, decrypted);
                            free(decrypted);
                        } else {
                            write(sockfd, "01", 2);  // Resend request
                        }
        ...
        case '3':  // Send
            ...
            if (pid_send == 0) {  // Child process
                int sockfd = create_socket(ip, port);
                char* encrypted = encrypt_and_sign_string(message);
                if (encrypted) {
                    write(sockfd, encrypted, strlen(encrypted));
                    free(encrypted);
                }
                close_socket(sockfd);
                exit(0);
